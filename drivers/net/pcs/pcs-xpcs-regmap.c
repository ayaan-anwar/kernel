...
static int xpcs_regmap_read_reg_indirect(struct dw_xpcs_regmap *pxpcs,
					 int dev, int reg)
{
	ptrdiff_t csr, ofs;
	unsigned int val;
...
static int xpcs_regmap_write_reg_indirect(struct dw_xpcs_regmap *pxpcs,
...
static int xpcs_regmap_read_reg_direct(struct dw_xpcs_regmap *pxpcs,
					int dev, int reg)
{
	unsigned int val;
	ptrdiff_t csr;
...
static int xpcs_regmap_write_reg_direct(struct dw_xpcs_regmap *pxpcs,
					int dev, int reg, u16 val)
{
				  ...
				  u16 val)
{
	...
	if (pxpcs->reg_indir)
		return xpcs_regmap_write_reg_indirect(pxpcs, MDIO_MMD_VEND2,
						      reg, val);
	else
		return xpcs_regmap_write_reg_direct(pxpcs, MDIO_MMD_VEND2,
						    reg, val);
}
