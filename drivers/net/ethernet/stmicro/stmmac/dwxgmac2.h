...
/* Timestamp / PPS registers — address-aware to support custom base offsets. */
...
static inline u32 xgmac_pps_base_addr(const struct dwxgmac_addrs *addrs, u32 x)
{
	return addrs ? addrs->pps_base + x * addrs->pps_offset
		     : XGMAC_PPS_BASE_ADDR + x * 0x10;
}
...
#define XGMAC_PPS_CONTROL(a)		(xgmac_timestamp_base_addr(a) + 0x70)
...
#define XGMAC_PPSx_TARGET_TIME_SEC(a, x)	xgmac_pps_base_addr(a, x)
#define XGMAC_PPSx_TARGET_TIME_NSEC(a, x)	(xgmac_pps_base_addr(a, x) + 0x04)
#define XGMAC_TRGTBUSY0			BIT(31)
#define XGMAC_PPSx_INTERVAL(a, x)	(xgmac_pps_base_addr(a, x) + 0x08)
#define XGMAC_PPSx_WIDTH(a, x)		(xgmac_pps_base_addr(a, x) + 0x0c)
...
/* MTL per-queue registers — address-aware to support custom queue offsets. */
...
/* DMA per-channel registers — address-aware to support custom channel offsets.
 * Nord SoC uses an odd/even interleaved base: even channels at dma_even_chan_base
 * + n*dma_chan_offset, odd channels at dma_odd_chan_base + n*dma_chan_offset.
 * All relative offsets within a channel are identical to the standard layout.
 */
